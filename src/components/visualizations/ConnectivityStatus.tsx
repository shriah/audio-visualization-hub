...
const ConnectivityStatus: React.FC<ConnectivityStatusProps> = ({ 
  ...
  // Get timing data for card display
  const networkTiming = preflightReport.report.networkTiming;
      ...
      <CardContent>
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="rounded-lg bg-secondary/50 p-3">
            ...
            <div className="font-medium">{networkTiming.dtls.duration}ms</div>
          </div>
          <div className="rounded-lg bg-secondary/50 p-3">
            <div className="text-muted-foreground mb-1 text-sm">ICE Connection</div>
            <div className="font-medium">{networkTiming.ice.duration}ms</div>
          </div>
          <div className="rounded-lg bg-secondary/50 p-3">
            <div className="text-muted-foreground mb-1 text-sm">Connection Time</div>
            <div className="font-medium">{networkTiming.connect.duration}ms</div>
          </div>
          <div className="rounded-lg bg-secondary/50 p-3">
            <div className="text-muted-foreground mb-1 text-sm">Media Setup</div>
            <div className="font-medium">{networkTiming.media.duration}ms</div>
          </div>
        </div>
